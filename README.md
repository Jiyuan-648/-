...
AI 生成的问题/缺陷：...（例如：没有使用面向对象/使用了没学过的库/没有捕获异常）

优化后的 Prompt (追问)：...（说明你增加了哪些约束条件）

## 3. Debug与异常处理记录

（记录一次解决报错或发现AI逻辑漏洞的过程）

报错类型/漏洞现象：...（如 FileNotFoundError 或 路径拼接错误）

解决过程：...（是我自己看懂了 Traceback 解决的，还是将报错喂给 AI 解决的？最终修改了哪里？）
